...
DIR_VIEWCLIENT = os.path.dirname(os.path.abspath(__file__))
sys.path.append(DIR_VIEWCLIENT + "\\adbclient\\")
import adbclient
#***********************************************************
...
class View:
    ...
    def __init__(self, map, device, version=-1, forceviewserveruse=False):
		...
		#
        self.build[VERSION_SDK_PROPERTY] = get_sdk_version(device, version)
        #++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
class UiAutomator2AndroidViewClient():
    ...
    def Parse(self, uiautomatorxml):
        ...
        # Parse the XML File
        try:
            parserStatus = parser.Parse(uiautomatorxml.encode('utf-8'), True)
...
class ViewClient:
    ...
    def __init__(self, device, serialno, adb=None, autodump=True, forceviewserveruse=False, localport=VIEW_SERVER_PORT, remoteport=VIEW_SERVER_PORT, startviewserver=True, ignoreuiautomatorkilled=False):
        ...
        #+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        self.build[VERSION_SDK_PROPERTY] = get_sdk_version(device, -1)
        #+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    #*******************************************
        ...
        #*******************************************
            ...
            DIR_VIEWCLIENT = os.path.dirname(os.path.abspath(__file__))
            possibleChoices.append(os.path.join(DIR_VIEWCLIENT + '\\..\\..\\..\\..\\Andorid_sdk\\platform-tools', adb))
        #*******************************************
    ...
    def serviceResponse(self, response):
        ...
        if DEBUG:
            print >>sys.stderr, "serviceResponse: comparing '%s' vs Parcel(%s)" % (str(response), str(PARCEL_TRUE))
...
#             self.traverse(ch, indent=indent+"   ", transform=transform, stream=stream)
    ...
    def dump(self, window=-1, sleep=1):
        ...
        #****************************************************************************************************
            if(str(received).startswith('Killed')):
                try:
                   self.device.getViewIdList()
                except:
                   #print "Unexpected error:", sys.exc_info()[0]
                   print "[viewclient] - Get view dump"
                   received = unicode(self.device.shell('uiautomator dump /dev/tty >/dev/null'), encoding='utf-8', errors='replace')
         #****************************************************************************************************
            if not received:
...
#====================================================================
def get_sdk_version(device, version):
    sdk_version = -1
    if version != -1:
        sdk_version = version
    else:
        try:
            if USE_ADB_CLIENT_TO_GET_BUILD_PROPERTIES:                    
                sdk_version = int(device.getProperty(VERSION_SDK_PROPERTY))
            else:
                sdk_version = int(device.shell('getprop ro.build.' + VERSION_SDK_PROPERTY)[:-2])
        except:
        #**********************************************************************
            #sdk_version = device.getProperty("build.version.sdk")
            sdk_version = os.popen('adb shell getprop ro.build.version.sdk').read().strip()
            if(sdk_version is None) or (not sdk_version.isdigit()): #20140911_ChirsHe - Add (not sdk_version.isdigit())
               sdk_version = -1
            else:
